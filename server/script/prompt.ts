...
- Stories are not isolated blocks. Do not default to story, transition, story, transition. When the plan connects two stories, you may tell them as one unfolding thread and let a segment list both ids. A callback is fine only to the same story or across a planned connection.
...
- Outro: make the ending intentional: return to the opening story's moment, leave a small takeaway that the evidence supports, or simply close naturally. Do not recap the stories, and do not introduce a thesis or a "what it all adds up to" that ties unrelated stories together.
